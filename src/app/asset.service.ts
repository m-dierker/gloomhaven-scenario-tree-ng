...
export class AssetService {
  ...
  public getScenariosJSON(): Observable<any> {
    ...
    return this.http.get<any>("./assets/scenarios.json").pipe(
      map((scenarios) => {
        ...
        scenarios.nodes = scenarios.nodes.sort(
          (n1, n2) => +n1.data.id - +n2.data.id
        );
        this.defaultScenariosJSON = cloneDeep(scenarios);
        if (encodedTree) {
          scenarios.nodes = this.getDecodedScenarios(
            scenarios.nodes,
            encodedTree
          ).nodes;
        }
  ...
  public getDecodedScenarios(currentNodes, savedScenarioString) {
    ...
    currentNodes.forEach((node, index) => {
      const savedNode =
        savedScenarios.nodes.find((saved) => saved.id === node.data.id) || {};
      const matchedBase = this.defaultScenariosJSON.nodes.find(
        (base) => base.data.id === node.data.id
      );
      /* If an attribute was saved then copy it over to the current full JSON */
      node.data.status =
        typeof savedNode.status !== "undefined"
          ? savedNode.status
          : matchedBase.data.status;
      node.data.notes =
        typeof savedNode.notes !== "undefined"
          ? savedNode.notes
          : matchedBase.data.notes;
      node.position.x =
        typeof savedNode.x !== "undefined"
          ? savedNode.x
          : matchedBase.position.x;
      node.position.y =
        typeof savedNode.y !== "undefined"
          ? savedNode.y
          : matchedBase.position.y;
      ...
      if (typeof savedNode.treasure !== "undefined") {
        ...
        Object.keys(matchedBase.data.treasure).forEach((number) => {
          node.data.treasure[number].looted =
            matchedBase.data.treasure[number].looted;
        });
      }
    });
  ...
  public getEncodedScenarios(scenarios) {
    ...
    const changedNodes = scenarios.nodes.reduce((changedNodes, node) => {
      ...
      Object.keys(matchedBase.data.treasure).forEach((number) => {
        if (
          matchedBase.data.treasure[number].looted.toString() !==
          node.data.treasure[number].looted.toString()
        ) {
          if (typeof simpleNode["treasure"] === "undefined") {
            simpleNode["treasure"] = {};
          }
          simpleNode["treasure"][number] = {
            looted: node.data.treasure[number].looted.toString(),
          };
        }
      });
  ...
  public setScenariosJSON(scenarios) {
    this.db.saveScenarios(this.getEncodedScenarios(scenarios));
    // localStorage.setItem(
    //   "gloomhavenScenarioTree",
    //   this.getEncodedScenarios(scenarios)
    // );
  }
