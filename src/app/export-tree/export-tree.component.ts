...
export class ImportExportDialogComponent {
  ...
  // encodedScenarios: Raw JSON.
  public encodedScenarios = new FormControl("", [
    Validators.required,
  ...
  constructor(
    ...
    this.encodedScenarios.setValue(
      this.assetService.getEncodedScenarios(this.scenarios)
    );
  }
  /** Converts encoded (JSON) to something else. */
  public importScenarios(): void {
    console.log("THIS SCENARIOS", this.scenarios);
    this.importError = null;
    try {
      const decodedScenarioJSON = this.assetService.getDecodedScenarios(
        this.scenarios.nodes,
        this.encodedScenarios.value
      );
  ...
  private validJSONValidator(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } => {
      return this.importError != null
        ? { validJSON: { value: this.importError } }
        : null;
    };
  }
}
